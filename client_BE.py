...
# 准备数据集 data_BE
def load_data_BE():
    # 假设 specific_length_dfs['data_BE'] 是已经处理好的数据集
    data_at = pd.read_csv('Preprocess_data/data_BE.csv')
    X, y = [], []
    window_size = 10
...
# Flower 客户端
class BEClient(fl.client.NumPyClient):
...
if __name__ == "__main__":
    # 使用 data_BE
    X_train, X_test, y_train, y_test = load_data_BE()
    
    # 不指定设备，默认使用 CPU
    model_BE = create_model_BE()
    client_BE = BEClient(model_BE, X_train, y_train, X_test, y_test)

    # 启动客户端
    fl.client.start_numpy_client(server_address="localhost:8080", client=client_BE)

    # 在训练完成后绘制并保存训练损失曲线
    client_BE.plot_training_loss()

    # 在训练完成后绘制并保存预测效果图和相对误差图
    client_BE.plot_predictions()
